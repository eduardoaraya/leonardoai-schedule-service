...
export interface IUserRepository {
  query: (request: IUserQueryRequest, select?: IUserSelect) => Promise<IUserResult[]>;
  create: (request: IUserCreateRequest, select?: IUserSelect) => Promise<IUserResult>;
  update: (id: number, request: IUserUpdateRequest, select?: IUserSelect) => Promise<IUserResult>;
  delete: (userId: number) => Promise<boolean>;
  getById: (userId: number, select?: IUserSelect) => Promise<IUserBase|null>;
}
export interface IUserService {
  filter: (request: Pick<IUserQueryRequest, "email"|"name">, select?: IUserSelect) => Promise<IUserResult[]>;
